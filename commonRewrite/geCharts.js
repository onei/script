/*global mw:true, Highcharts:true, addCommas:true, window:true, $:true, alert:true, location:true, document:true, setTimeout:true */
/*jshint devel:true, jquery: true, browser:true, strict:false */
(function () {

    var newhash = location.hash.replace(/\.([0-9a-f]{2})/gi, function (first) {
...
function getData(c, isSmall, avginput) {
    ...
    itemName = itemName || mw.config.get('wgPageName').replace(/_/g, ' ').split('/')[0].replace('Exchange:', '');
    avginput = avginput || c;
    for (i = 0; i < prices.length; i += 1) {
        if (prices[i].replace(/\s/g, '').length) {
            thisprice = prices[i].split(':');
            data.push([ parseInt(thisprice[0], 10) * 1000, parseFloat(thisprice[1]) ]);
            if (thisprice[2] && !isSmall) {
                volumes.push([ parseInt(thisprice[0], 10) * 1000, parseFloat(thisprice[2]) * 1000000 ]); // volumes are in millions
            }
        }
    }
...
function reloadChart(c, change) {
    ...
    if (!_GEC['chart' + c].options) {
        return false;
    }
    for (prop in change) {
        if (change.hasOwnProperty(prop)) {
            _GEC['chart' + c].options[prop] = change[prop];
        }
    }
...
function chartPermLinkUrl() {
    var xt = _GEC.chartpopup.xAxis[0].getExtremes(),
        ...
        inputAvg = parseInt($('#averagepopup').val(), 10),
        items = '',
        urlHash = '#t=' + minDate + ',' + maxDate;


    for (i = 0; i < _GEC.chartpopup.series.length; i += 1) {
        if (_GEC.chartpopup.series[i].name === 'Navigator' || _GEC.chartpopup.series[i].name.match('average')) {
            continue;
        }
...
function setChartExtremes(i) {
    var extremes = _GEC['chart' + i].yAxis[0].getExtremes();
    ...
    if (i === 'popup') {
        $('#GEPermLink' + i).get(0).href = chartPermLinkUrl();
    }
}
...
function removeGraphItem(item, c) {
    ...
    for (i = 0; i < _GEC['chart' + c].series.length; i += 1) {
    ...
    if (!$('#addedItems' + c + ' [data-item]').length) {
        $('#addedItems' + c).empty();
        id = c === 'popup' ? $('#GEpopupchart').attr('data-chartid') : c;
...
function setChartRange(c) {
    var zoom = parseInt((location.hash.match(/#z=([^#]*)/) || [])[1], 10),
        hash = location.hash,
        hasT = (mw.config.get('wgNamespaceNumber') === 112 && mw.config.get('wgTitle').split('/')[1] === 'Data') || mw.config.get('wgPageName') === 'Grand_Exchange_Market_Watch/Chart',
        timespan = unescape((hash.match(/#t=([^#]*)/) || [])[1] || '').replace(/_/g, ' ').split(','),
        dates = [new Date(timespan[0]), new Date(timespan[1])],
        d = new Date(timespan[0]),
        extremes = _GEC['chart' + c].xAxis[0].getExtremes(),
        button;
    ...
    if (dates[0] !== 'Invalid Date' && dates[1] === 'Invalid Date' && typeof zoom === 'number') {
        button = _GEC['chart' + c].rangeSelector.buttonOptions[zoom];
...
function loadChartsQueueComplete(c) {
    ...
    isSmall = $('#GEdatachart' + c).is('.smallChart');
    data = [_GEC.AIQueue.length];

    for (i = 0; i < _GEC.AIData.length; i += 1) {
        index = _GEC.AIQueue.indexOf((_GEC.AIData[i] || {
            name: ''
        }).name.toLowerCase());
    ...
    for (i = 0; i < data.length; i += 1) {
        ...
        if (isSmall) {
            if (typeof _GEC.addedData[c] !== 'object') {
                _GEC.addedData[c] = [];
            }
    ...
    if (((mw.config.get('wgNamespaceNumber') === 112 && mw.config.get('wgTitle').split('/')[1] === 'Data') || mw.config.get('wgPageName') === 'Grand_Exchange_Market_Watch/Chart') && itemhash.replace('#i=', '').length) {
...
function addItem(i, it) {
    var id,
        item = it || $('#extraItem' + _GEC.chartid).val(),
        data,
        series;
    ...
    if (item && item.length) {
        if ($('#addedItems' + i + ' [data-item], #GEdataprices' + i + '[data-item]').map(function () {
            return $(this).attr('data-item').toLowerCase();
        }).get().indexOf(item.toLowerCase()) !== -1 || _GEC.AIQueue.indexOf(item.toLowerCase()) !== -1) {
            if (!it) {
                alert(item + ' is already in the graph.');
            }
            $('#extraItem' + _GEC.chartid).val('');
            return false;
        }
        ...
        $.get('/api.php?action=parse&format=json&page=Exchange:' + encodeURIComponent(item) + '/Data', function (response) {
            ...
            if (response.error) {
                item = decodeURIComponent((this.url.match(/Exchange:([^\/]*)\/Data/) || [])[1]);
            ...
            for (i = 0; i < prices.length; i += 1) {
                if (prices[i].replace(/\s/g, '').length) {
                    thisprice = prices[i].split(':');
                    data.push([ parseInt(thisprice[0], 10) * 1000, parseInt(thisprice[1], 10) ]);
                }
            }
            ...
            if (getSeriesIndex(_GEC.chartid, 'average') !== -1) {
                _GEC['chart' + _GEC.chartid].series[getSeriesIndex(_GEC.chartid, 'average')].remove();
            }

            if (_GEC.AIQueue.length === _GEC.AILoaded.length) {
                loadChartsQueueComplete(_GEC.chartid); //This is always true when only 1 item is being loaded.
            }
        });
        ...
        if ($('#addedItems' + i).html().match(/^\s*$/)) {
            ...
            if (getSeriesIndex(i, 'average') !== -1) {
                series[getSeriesIndex(i, 'average')].remove();
            }
...
function addPopupPriceInfo(c) {
    ...
    var prices,
        ...
        priceDiffs = [],
        percentDiffs = [],
        dayDiffs = [1, 7, 30, 90, 180],
        itemStats,
        pthen,
        pdiff;

    if (($('#GEdataprices' + c).attr('data-item') || '').match(/index/i)) {
        return false;
    }
    prices = $('#GEdataprices' + c).html().split(/,\s/);
    ...
    for (i = 0; i < prices.length; i += 1) {
        if (prices[i].replace(/\s/g, '').length) {
            ...
            data.push(parseInt(curprice[1], 10));
        }
    }
    ...
    for (i = 0; i < dayDiffs.length; i += 1) {
        pthen = data[datal - dayDiffs[i] - 1];
        pdiff = curprice - pthen;
        if (!isNaN(pdiff)) {
    ...
    itemStats = '<table class="wikitable">' + '<caption onclick="$(this).next(\'tbody\').toggle();$(this).children().toggle()"><span style="display:none;white-space:nowrap">Price Info</span><span>Close</span></caption><tbody>' + '<tr><th>Price</th><td>' + toKMB(curprice) + '</td></tr>';

    for (i = 0; i < priceDiffs.length; i += 1) {
...
function popupChart(i) {
    ...
    if (typeof i === 'number' && $('#GEpopupchart').attr('data-chartid') !== i) {
        ...
        if (_GEC.addedData[i]) {
            for (n = 0; n < _GEC.addedData[i].length; n += 1) {
...
$('div.GEdatachart').each(function (c) {
    ...
    if (itemName.toLowerCase() !== 'blank') {
        zoom = parseInt((location.hash.match(/#z=([^#]*)/) || [])[1], 10);
        zoom = zoom && zoom <= 6 && zoom >= 0 ? zoom - 1 : (zoom === 0 ? 0 : 2);
    }
    enlarge = '<a href="javascript:popupChart(' + c + ');" style="text-decoration:underline;">Enlarge chart</a>';
    ...
    _GEC['chart' + c] = new Highcharts.StockChart({
        ...
        rangeSelector: {
            ...
            buttonTheme: {
                class: 'zoomButton' // @todo ask juey about this, class is a res. word -> className
            },
            buttons: [{
                type: 'month',
                count: 1,
    ...
    items = ($('#GEChartItems' + c).html() || '').split(',');
    noAdd = [];
    ...
    if (items.length === noAdd.length && _GEC['chart' + c].series[0].name.toLowerCase() !== 'blank') {
        setChartRange(c);
    }
    ...
    if (((mw.config.get('wgNamespaceNumber') === 112 && mw.config.get('wgTitle').split('/')[1] === 'Data') || mw.config.get('wgPageName') === 'Grand_Exchange_Market_Watch/Chart') && location.hash.match('#i=')) {
        hash = location.hash;
